...
#if FUNC_REC_EN
rec_src_t rec_src AT(.record.src);
rec_cb_t  rec_cb AT(.record.cb);
au_stm_t rec_enc_stm AT(.record.stm);
au_stm_t rec_pcm_stm AT(.record.stm);
u8 mp3enc_flag AT(.buf.record);
...
{
#if FUNC_REC_EN
    puts_stm_buf(rec_cb.pcm, inbuf, len);
#if (REC_TYPE_SEL == REC_WAV)
    music_mq_post(ENC_MSG_WAV);
#elif (REC_TYPE_SEL == REC_ADPCM)
    music_mq_post(ENC_MSG_ADPCM);
#elif (REC_TYPE_SEL == REC_MP3)
...
{
    return 160000;  //32000, 48000, 64000, 96000, 128000, 160000;
}
...
#if REC_AUTO_END_EN
    //delete all record file
    printf("delete all record file\n");
    // music_control(MUSIC_MSG_STOP);
    u32 addr = 0, len = 0;
    u8 recRet = 1;
    u32 rec_total = spiflash_rec_get_total_file();
    //printf("rec_total 1 = %d\n",rec_total);
    if(rec_total != 0){
        for(u8 i = 0; i < rec_total; i++){
            spiflash_rec_delete_curplay_file();
            recRet = spiflash_rec_open_next_file(&addr, &len);
            if(recRet != 0){
                break;
            }
        }
    }
    rec_total = spiflash_rec_get_total_file();
    //printf("rec_total 2 = %d\n",rec_total);
#endif
